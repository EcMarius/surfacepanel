...
@section('title', 'Edit Email Filter')
...
<div class="container mx-auto px-4 py-8">
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 mb-2">Edit Email Filter</h1>
                <p class="text-gray-600">Modify your filter rule configuration</p>
            </div>
            <a href="{{ route('user.email-filters.index', ['email' => $filter->email]) }}" class="text-gray-600 hover:text-gray-900 transition">
    ...
    <form method="POST" action="{{ route('user.email-filters.update', $filter->id) }}" id="filterForm">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-lg shadow-lg mb-6 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="email" id="email" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        @foreach($emailAccounts as $emailAccount)
                            <option value="{{ $emailAccount }}" {{ old('email', $filter->email) === $emailAccount ? 'selected' : '' }}>
                                {{ $emailAccount }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $filter->name) }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    @error('name')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mt-4">
                ...
                <textarea name="description" id="description" rows="2" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('description', $filter->description) }}</textarea>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
                <div>
                    <label for="priority" class="block text-sm font-medium text-gray-700 mb-2">Priority</label>
                    <input type="number" name="priority" id="priority" value="{{ old('priority', $filter->priority) }}" min="1" max="999" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div>
                    ...
                    <select name="match_type" id="match_type" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        <option value="all" {{ old('match_type', $filter->match_type) === 'all' ? 'selected' : '' }}>Match ALL conditions</option>
                        <option value="any" {{ old('match_type', $filter->match_type) === 'any' ? 'selected' : '' }}>Match ANY condition</option>
                    </select>
                </div>

                <div class="flex items-center pt-8">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $filter->is_active) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
        ...
        <div class="flex justify-end space-x-4">
            <a href="{{ route('user.email-filters.index', ['email' => $filter->email]) }}" class="px-6 py-3 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition font-medium">
                Cancel
            </a>
...
const existingConditions = @json($filter->conditions);
const existingActions = @json($filter->actions);

// Load existing conditions and actions
                ...
                <div>
                    ...
                    <select name="conditions[${conditionIndex}][field]" onchange="updateConditionUI(this)" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                        ${Object.entries(availableFields).map(([key, value]) =>
                            `<option value="${key}" ${data && data.field === key ? 'selected' : ''}>${value}</option>`
                        ).join('')}
                    </select>
                </div>
                <div class="header-name-field ${data && data.field === 'header' ? '' : 'hidden'}">
                    <label class="block text-xs font-medium text-gray-700 mb-1">Header Name</label>
                    <input type="text" name="conditions[${conditionIndex}][header_name]" value="${data?.header_name || ''}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="X-Custom-Header">
                </div>
                <div>
                    ...
                    <select name="conditions[${conditionIndex}][operator]" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                        ${Object.entries(availableOperators).map(([key, value]) =>
                            `<option value="${key}" ${data && data.operator === key ? 'selected' : ''}>${value}</option>`
                        ).join('')}
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Value</label>
                    <input type="text" name="conditions[${conditionIndex}][value]" value="${data?.value || ''}" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="Enter value...">
                </div>
            </div>
            <div class="mt-3">
                <label class="flex items-center">
                    <input type="checkbox" name="conditions[${conditionIndex}][case_sensitive]" value="1" ${data && data.case_sensitive ? 'checked' : ''} class="w-4 h-4 text-blue-600 border-gray-300 rounded">
...
function addAction(data = null) {
    ...
    const isVacation = data && data.action_type === 'vacation';
    const requiresValue = data && ['move_to_folder', 'forward', 'redirect', 'reject', 'pipe_to_program'].includes(data.action_type);
                ...
                <div>
                    ...
                    <select name="actions[${actionIndex}][action_type]" onchange="updateActionUI(this)" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                        ${Object.entries(availableActions).map(([key, value]) =>
                            `<option value="${key}" ${data && data.action_type === key ? 'selected' : ''}>${value}</option>`
                        ).join('')}
                    </select>
                </div>
                <div class="action-value-field ${!isVacation && requiresValue ? '' : 'hidden'}">
                    <label class="block text-xs font-medium text-gray-700 mb-1">Value</label>
                    <input type="text" name="actions[${actionIndex}][action_value]" value="${data?.action_value || ''}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="Enter value...">
                </div>
            </div>
            <div class="vacation-fields ${isVacation ? '' : 'hidden'} mt-4">
                <div class="grid grid-cols-1 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Vacation Subject</label>
                        <input type="text" name="actions[${actionIndex}][vacation_subject]" value="${data?.vacation_subject || ''}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="Out of Office">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Vacation Message</label>
                        <textarea name="actions[${actionIndex}][vacation_message]" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="I am currently out of office...">${data?.vacation_message || ''}</textarea>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Days Between Responses</label>
                        <input type="number" name="actions[${actionIndex}][vacation_days]" value="${data?.vacation_days || 7}" min="1" max="30" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                    </div>
                </div>
            </div>
        </div>
